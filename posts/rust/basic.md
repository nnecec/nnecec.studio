...
- 使用 `cargo new` 创建一个 Rust 项目，在项目路径下可以执行 `cargo build` 编译，执行 `cargo run`
  运行，执行 `cargo check` 检查是否有编译错误。
...
- 枚举同样可以通过 `impl` 声明方法。
- Rust 中没有空值，但可以通过 `Option<T>` 的枚举值 `None` 实现类似空值的能力，同时 `Option<T>`
  有值时的枚举类型是 `Some<T>`。
...
- 当需要 `use` 同一个包内的多个模块时，可以使用 `{}` 将同一大模块下的子模块合并。或使用 `*`
  通配符导入所有子模块，但尽量谨慎使用。
...
- `v.get(2)` 会返回 `Option<&T>` 类型的值，另一种获取元素的方法 `&v[2]`
  如果索引超出了边界则会直接触发 `panic`
...
- 使用 `unwrap` 在正确时返回值，错误时默认调用 `panic!`。 `expect` 与 `unwrap`
  功能类似，但可以带上一段错误提示信息。
- 使用 `Err(e)` 包装将错误返回给用户自由处理，使用 `?` 运算符可以简写。 `?`
...
- 被编码进 Rust 引用分析的模式被称为  **生命周期省略规则**（_lifetime elision
  rules_），如果 Rust 在明确遵守这些规则的前提下变量的生命周期仍然是模棱两可的话，它不会猜测剩余引用的生命周期应该是什么。
- 函数或方法的参数的生命周期被称为  **输入生命周期**（_input
  ...
  3.  如果方法有多个输入生命周期参数并且其中一个参数是   `&self`   或  
...
- 通过 `#[should_panic]` 属性标注函数应该 panic，通过 `#[should_panic(expect = "error message")]`
  提示 panic 信息。
...
- 通过 `#[ignore]` 标记忽略测试该函数，再通过 `cargo test -- --ignored`
  可以忽略标记了 ignore 的函数。
...
- 迭代器实现了名为 `Iterator` 的 trait， `next` 是唯一被要求定义的方法。 `next`
  一次返回迭代器中的一个值 `Some<T>`，结束时返回 `None`
- `next`   调用中得到的值是 vector 的不可变引用
- **迭代器适配器**（_iterator
  adaptors_），他们允许我们将当前迭代器变为不同类型的迭代器，迭代器适配器是惰性的，而这里我们需要消费迭代器。
...
- 自定义类型需要通过 `Deref` trait 实现，实现了 `Deref`
...
- 在单线程场景中，通过 `Rc<T>` 分配多所有权， `Rc::new()` 创建引用， `Rc::clone()`
  复制引用，当引用计数为 0 时会清理内存并处罚 `Drop` 。
