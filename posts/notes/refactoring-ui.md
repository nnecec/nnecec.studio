...
例如在产品规格页面，有大量的规格属性需要展示，且为了让用户可以直接搜索，此时仍需要使用明确的标签，例如 iPhone 的规格页面会展示
...
WCAG 建议小于 18px 的字体 [对比度](https://webaim.org/resources/contrastchecker/)
至少是 4.5:1，大于 18px 的字体对比度至少是 3:1。
...
- 不要放大到图片模糊
- 如果图片太小可以将图片放到轮廓图形中
- 不要缩小屏幕截图，会使截图难以阅读。超出范围或者放大局部都是更好的选择。如果真的需要放置缩小的截图，可以简化界面例如删除截图中的小文字。
...
设计数据空状态的界面时，如果数据依赖用户输入，那么在提示用户当前界面无数据的情况时，同时提供新增数据的按钮引导用户添加数据。
