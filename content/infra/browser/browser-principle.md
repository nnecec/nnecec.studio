...
网络协议分层从下往上依次为物理层、数据链路层、网络层、传输层、会话层、表示层、应用层。TCP/IP 协议将应用层、表示层、会话层合并为应用层，物理层和数据链路层合并为网络接口层。
...
- 三次握手的必要性
  ...
  如果 2 次，则 Server 无法确认

- 半连接队列，全连接队列

  半连接队列指 Server 第一次收到 SYN 之后，就会处于 SYN_RCVD 状态的连接的队列。全连接队列指完成三次握手的连接队列。
...
1. Client 发起握手请求，并携带支持的 `TLS 版本`、`加密方案`，以及一个随机字符 `ClientHello`
2. Server 发送响应请求，确认选择支持的`TLS 版本`、`加密方案`，以及一个随机字符 `ServerHello`、`数字证书(公钥)`
3. Client 检查数字证书有效期、有效状态，发送一个经过公钥加密的随机数`pre-master key`、`编码改变通知`，
4. Client 发送经过 KEY 加密过的`finished`信号，此时 Client 就绪
5. Server 根据 KEY 解密`finished`信号验证，最后一次发送 KEY 加密过的`finished`信号、`编码改变通知`，Server 就绪

> KEY 由 `ClientHello` + `ServerHello` + `pre-master key` 通过加密方法生成会话密钥
