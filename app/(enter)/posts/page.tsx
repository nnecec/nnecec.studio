          ...
          return (
            <Link className="no-underline" href={`/posts${post.slug}`} key={post.slug}>
              <div className="group relative mb-4 break-inside-avoid-column space-y-2 rounded-xl border bg-background p-4 hover:bg-foreground/5">
                <h3 className="my-2! group-hover:text-primary">{post.title}</h3>
                ...
                <Button
                  className="invisible absolute right-3 top-5 -translate-x-3 opacity-0 transition group-hover:translate-x-0 group-hover:opacity-100 md:visible"
                  size="icon"
                  variant="ghost"
                >
                  <ChevronRightIcon />
